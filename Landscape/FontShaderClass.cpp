...
bool FontShaderClass::Initialize(ID3D10Device* device, HWND hwnd)
{
	...
	if(!InitializeShader(device, hwnd, "Data/Shaders/FontShader.fx"))
		return false;
...
void FontShaderClass::Render(ID3D10Device* device, long indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, 
	ID3D10ShaderResourceView* texture, D3DXCOLOR fontColor)
{
	// Set the shader parameters that it will use for rendering.
	SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texture, fontColor);
...
bool FontShaderClass::InitializeShader(ID3D10Device* device, HWND hwnd, char* filename)
{
	...
	// Get pointers to the variables inside the shader
    m_worldMatrixPtr = m_effect->GetVariableByName("worldMatrix")->AsMatrix();
	m_viewMatrixPtr = m_effect->GetVariableByName("viewMatrix")->AsMatrix();
    m_projectionMatrixPtr = m_effect->GetVariableByName("projectionMatrix")->AsMatrix();
	m_texturePtr = m_effect->GetVariableByName("fontTexture")->AsShaderResource();
	m_fontColorPtr = m_effect->GetVariableByName("fontColor")->AsVector();
	...
	ID3D10ShaderResourceView* texture, D3DXCOLOR fontColor)
{
	...
	// Sets the texture inside the shader.
	m_texturePtr->SetResource(texture);

	// Set the font color
	m_fontColorPtr->SetFloatVector((float*)fontColor);

	return;
}

void FontShaderClass::RenderShader(ID3D10Device* device, long indexCount)
{
