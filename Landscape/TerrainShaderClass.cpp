...
bool TerrainShaderClass::Initialize(ID3D10Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, "Data/Shaders/TerrainShader.fx");
	if(!result)
	{
		return false;
	}
...
bool TerrainShaderClass::InitializeShader(ID3D10Device* device, HWND hwnd, LPCSTR filename)
{
	HRESULT result;
