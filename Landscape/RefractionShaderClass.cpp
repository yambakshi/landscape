...
bool RefractionShaderClass::Initialize(ID3D10Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, "Data/Shaders/RefractionShader.fx");
	if(!result)
	{
		return false;
	}
...
bool RefractionShaderClass::InitializeShader(ID3D10Device* device, HWND hwnd, LPCSTR filename)
{
	HRESULT result;
...
void RefractionShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCSTR shaderFilename)
{
	char* compileErrors;
